...
def _get_root() -> Path:
    return Path(__file__).resolve().parents[1]  # adjust if needed
...
def corrupt_mnist() -> tuple[torch.utils.data.Dataset, torch.utils.data.Dataset]:
    ...
    train_images = torch.load(root / "data/processed/train_images.pt")
    train_target = torch.load(root / "data/processed/train_target.pt")
    test_images = torch.load(root / "data/processed/test_images.pt")
    test_target = torch.load(root / "data/processed/test_target.pt")
